...
const makeQueryFeatureStages = (queryFeatures: IQueryFeatures, options: IOptions): PipelineStage[] => {

    const fieldsSelectionStage: PipelineStage = Object.keys(queryFeatures.fields).length > 0 ? { $project: queryFeatures.fields } : {
        $addFields: {}
    }
