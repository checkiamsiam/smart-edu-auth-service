...
const updateSemesterReq = z.object({
  body: z.object({
    title: z.enum([...academicSemesterTitles] as [string, ...string[]], {
      required_error: "Title is must required",
      invalid_type_error: "Title must be a string",
  ...
  if (value.body.title && value.body.code) {
    return academicSemesterTitleCodeMapper[value.body.title] === value.body.code
  }else{
    return true
  }
}, { message: "title and code are not matching" });

const academicSemesterValidation = { createAcademicSemesterReq, updateSemesterReq };
